...
- **Circles** still uses mock rooms and participants.
  ...
  - Fix: wire to real rooms or **feature‑gate** until backend exists. (Now gated behind `FEATURE_GATES.CIRCLES`.)
...
- **Browse Schools** and **Search Forums** are placeholders.
  ...
  - Fix: connect to Supabase universities and forums search, or **feature‑gate** until ready. (Now gated behind `FEATURE_GATES.BROWSE_SCHOOLS` and `FEATURE_GATES.SEARCH_FORUMS`.)
...
- **Rate Professor** is not wired and review submission is TODO.
  ...
  - Fix: ship only after backend is ready or keep **feature‑gated** (already gated via `FEATURE_GATES.RATE_MY_PROFESSOR`).
