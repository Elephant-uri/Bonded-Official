...
import YearbookProfileModalContent from '../YearbookProfileModalContent'
                ...
                <PostAvatar 
                    ...
                    onPress={() => {
                        if (!comment.isAnon && comment.userId) {
                            onPressProfile?.(comment.userId, comment.isAnon)
                        }
                    }}
                />
            </View>
...
export default function ForumPostDetail({
    ...
    // Profile modal state (rendered inside this component)
    const profileTranslateY = useRef(new Animated.Value(SCREEN_HEIGHT)).current
    const [isProfileVisible, setIsProfileVisible] = useState(false)
    
    useEffect(() => {
        if (activeProfileId) {
            setIsProfileVisible(true)
            profileTranslateY.setValue(SCREEN_HEIGHT)
    ...
    // Cleanup: Clear profile when post is closed/unmounted
    useEffect(() => {
        if (!post) {
            setIsProfileVisible(false)
            setActiveProfileId?.(null)
        }
    }, [post, setActiveProfileId])
    ...
    }, [setActiveProfileId, profileTranslateY])
    ...
    const profileBackdropOpacity = profileTranslateY.interpolate({
    ...
    return (
                                    ...
                                    <PostAvatar 
                                        ...
                                        onPress={() => {
                                            if (!post.isAnon && post.userId) {
                                                onPressProfile?.(post.userId, post.isAnon)
                                            }
                                        }}
                                    />
                                    <TouchableOpacity
        ...
        {/* Profile Modal - rendered inside ForumPostDetail */}
        {isProfileVisible && (
            <>
                ...
                >
                    {profileLoading || !activeProfile ? (
                        ...
                        <YearbookProfileModalContent
                            ...
                            setActiveProfile={handleCloseProfile}
                            theme={theme}
                            router={router}
                            currentUserInterests={currentUserInterests || new Set()}
                            onClose={handleCloseProfile}
                            panResponder={profilePanResponder}
                        />
                    )}
                </Animated.View>
            </>
